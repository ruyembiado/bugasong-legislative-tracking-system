                ...
                switch ($validationType) {
                    ...
                    case 'unique':
                        if (is_array($validationValue)) {
                            foreach ($validationValue as $uniqueValidation) {
                                if (isset($uniqueValidation['fieldName']) && isset($uniqueValidation['tableName'])) {
                                    if (!isUnique($conn, $uniqueValidation['fieldName'], $uniqueValidation['tableName'], $fieldValue)) {
                                        if (!isset($errors[$fieldName])) {
                            ...
                            trigger_error("unique validation for field {$fieldName} must be an array of arrays", E_USER_WARNING);
                        }
                        break;
                }
            }
